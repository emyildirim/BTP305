	...
	struct Profile {
		...
		void validateAddress() {
			if ((this->m_address.number < 0
				|| this->m_address.postal_code.length() <= 0
				|| std::isdigit(this->m_address.postal_code[0])
				|| !std::isdigit(this->m_address.postal_code[1])
				|| std::isdigit(this->m_address.postal_code[2])
				|| !std::isdigit(this->m_address.postal_code[4])
				|| std::isdigit(this->m_address.postal_code[5])
				|| !std::isdigit(this->m_address.postal_code[6]))
				&& this->m_address.postal_code[3] == ' ') {
