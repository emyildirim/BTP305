...
int main(int argc, char** argv)
{
	...
	try {
		for (auto i = 0u; i < 10; ++i) {
			if (i >= theCollection.size()) {
				throw std::out_of_range("Bad index [" + std::to_string(i) + "]. Collection has [" + std::to_string(theCollection.size()) + "] items.");
				break;
			}
